...
init([Id,Opts]) ->
    ...
    if File =:= undefined ->
	    lager:error("nmea_2000_log: missing file argument"),
	    {stop, einval};
       true ->
	    ...
	    LogFile = nmea_2000_lib:text_expand(File,[]),
	    case join(Router, Pid, {?MODULE,File,Id,Name}) of
...
open_logfile(S0=#s {file = File }) ->
	...
	{error,E} when E =:= eaccess; E =:= enoent ->
	    lager:debug("error ~w, will try again in ~p msecs.", 
